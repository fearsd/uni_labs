...
namespace lab7_first {
    class Helper {
        ...
        // }
        public static void countMatch(Match match, List<TeamExtended> teams, bool isGuest=false) {
            ...
            if (queryResult != 0) {
                TeamExtended team = teams.Where(t => t.name == teamName).ToArray()[0];
                team.Update(match.scoredGoals, match.missedGoals, getScore(match.scoredGoals, match.missedGoals));
            }
            else {
                teams.Add(new TeamExtended(teamName, match.scoredGoals, match.missedGoals, getScore(match.scoredGoals, match.missedGoals)));
            }
        }
        public static int getScore(int scoredGoals, int missedGoals) {
    ...
    class TeamExtended : Team {
        public TeamExtended(string name, int scoredGoals, int missedGoals, int score) : base(name, scoredGoals, missedGoals, score) {}
        ...
        public static void SortTeams(List<TeamExtended> teams) {
            int n = teams.Count();
            TeamExtended temp;
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n - 1; j++) {
    ...
    class OptionAnswerExtended : OptionAnswer {
        public OptionAnswerExtended(string answer, int num, int answerNum) : base(answer, num, answerNum) {}
        public static void conductVotes(Answer[] answers, List<OptionAnswerExtended> optionAnswers) {
            foreach (Answer ans in answers) {
                ...
                if ((ans.answer1 != "") && isAnswerInList1) {
                    optionAnswers.Add(new OptionAnswerExtended(ans.answer1, 1, 1));
                }
                else if ((ans.answer1 != "") && !isAnswerInList1) {
                    OptionAnswerExtended optAnswer = optionAnswers.Where(_ans => (_ans.answer == ans.answer1) && (_ans.answerNum == 1)).ToList()[0];
                    optAnswer.num += 1;
                }

                if ((ans.answer2 != "") && isAnswerInList2) {
                    optionAnswers.Add(new OptionAnswerExtended(ans.answer2, 1, 2));
                }
                else if ((ans.answer2 != "") && !isAnswerInList2) {
                    OptionAnswerExtended optAnswer = optionAnswers.Where(_ans => (_ans.answer == ans.answer2) && (_ans.answerNum == 2)).ToList()[0];
                    optAnswer.num += 1;
                }

                if ((ans.answer3 != "") && isAnswerInList3) {
                    optionAnswers.Add(new OptionAnswerExtended(ans.answer3, 1, 3));
                }
                else if ((ans.answer3 != "") && !isAnswerInList3) {
                    OptionAnswerExtended optAnswer = optionAnswers.Where(_ans => (_ans.answer == ans.answer3) && (_ans.answerNum == 3)).ToList()[0];
                    optAnswer.num += 1;
                }
            }
        }
        public static void countProcent(OptionAnswerExtended[] optionAnswers) {
            int sum = 0;
            foreach (OptionAnswerExtended ans in optionAnswers)
            {
                sum += ans.num;
            }
            foreach (OptionAnswerExtended ans in optionAnswers)
        ...
        public static void printResultVotes(OptionAnswerExtended[] optionAnswers) {
            foreach (OptionAnswerExtended ans in optionAnswers)
            {
    ...
    class Program {
        static void Main(string[] args) {
            ...
            foreach (Match m in matches) {
                Helper.countMatch(m, teams);
                Helper.countMatch(m, teams, isGuest: true);
            }
            TeamExtended.SortTeams(teams);
            ...
            List<OptionAnswerExtended> optionAnswers = new List<OptionAnswerExtended>();
            OptionAnswerExtended.conductVotes(answers, optionAnswers);
            OptionAnswerExtended[] optionAnswers1 = optionAnswers.Where(ans => ans.answerNum == 1).OrderBy(ans => ans.num).Reverse().Take(5).ToArray();
            OptionAnswerExtended.countProcent(optionAnswers1);
            OptionAnswerExtended[] optionAnswers2 = optionAnswers.Where(ans => ans.answerNum == 2).OrderBy(ans => ans.num).Reverse().Take(5).ToArray();
            OptionAnswerExtended.countProcent(optionAnswers2);
            OptionAnswerExtended[] optionAnswers3 = optionAnswers.Where(ans => ans.answerNum == 3).OrderBy(ans => ans.num).Reverse().Take(5).ToArray();
            OptionAnswerExtended.countProcent(optionAnswers3);


            Console.WriteLine("Первый вопрос");
            OptionAnswerExtended.printResultVotes(optionAnswers1);
            Console.WriteLine("Второй вопрос");
            OptionAnswerExtended.printResultVotes(optionAnswers2);
            Console.WriteLine("Третий вопрос");
            OptionAnswerExtended.printResultVotes(optionAnswers3);
        }
    }
